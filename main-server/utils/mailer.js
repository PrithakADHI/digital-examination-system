...
function buildPasswordResetOtpEmail({ fullName, otp, expiresInMinutes }) {
  ...
  <div style="font-family: Arial, sans-serif; background: #f4f7fb; padding: 24px;">
    <div style="max-width: 620px; margin: 0 auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
      <div style="background: linear-gradient(90deg, #0f766e, #0369a1); color: #ffffff; padding: 18px 24px;">
        <h2 style="margin: 0; font-size: 20px;">${APP_NAME}</h2>
        <p style="margin: 6px 0 0; font-size: 13px;">Password reset request</p>
      </div>
      <div style="padding: 24px; color: #1f2937;">
        <p style="margin-top: 0;">Hello ${fullName || "User"},</p>
        <p style="line-height: 1.55; margin: 0 0 16px;">
        ...
        <div style="background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 10px; padding: 14px 16px; margin-bottom: 16px; text-align: center;">
          <p style="margin: 0 0 8px; color: #475569; font-size: 13px;">Your OTP</p>
...
export async function sendPasswordResetOtpEmail({ to, fullName, otp, expiresInMinutes = 10 }) {
  ...
  await transporter.sendMail({
    ...
    html: buildPasswordResetOtpEmail({ fullName, otp, expiresInMinutes }),
  });
}
